...
public class LoginEventHandler extends BaseServerEventHandler 
{
	...
	public void handleServerEvent(ISFSEvent event) throws SFSException
	{
		...
		if( CORE_SIZE == 0 )
		{
			try { CORE_SIZE = (int) HttpClients.createDefault().execute(new HttpGet("http://localhost:8080/swfcores/core-" + loginData.coreVersion + ".swf")).getEntity().getContentLength();
	...
	private void initiateCore(ISession session, ISFSObject inData, ISFSObject outData, LoginData loginData)
	{
		...
		boolean hasDonationR = false;
		boolean hasDonationD = false;
		for(int i=0; i<elements.size(); i++)
		{
			element = elements.getSFSObject(i);
			...
			if( ct == ExchangeType.CHEST_CATE_110_BATTLES )
				hasNewChests = true;
			if( t == ExchangeType.DONATION_141_REQUEST )
				hasDonationR = true;
			if( t == ExchangeType.DONATION_142_DONATE )
				hasDonationD = true;

			initData.exchanges.set( t, new Exchange( t, element.getInt("num_exchanges"), element.getInt("expired_at"), element.getInt("outcome")));
		}

		trace("hasDonation(R-D):", hasDonationR, hasDonationD);
		...
		if( !hasNewChests )
		{
			for (int i = 1; i <= 3 ; i++)
			{
				addNewExchangeElement(ExchangeType.CHEST_CATE_110_BATTLES + i, elements, newExchanges, initData, now);
				addNewExchangeElement(ExchangeType.CHEST_CATE_120_OFFERS + i, elements, newExchanges, initData, now);
			}
		}

		// add new free books for old players --> backward compatibility
		if( !has101 )
			addNewExchangeElement(ExchangeType.CHEST_CATE_101_FREE, elements, newExchanges, initData, now );
		if( !has102 )
		...
		if( !hasDonationD )
			addNewExchangeElement(ExchangeType.DONATION_142_DONATE, elements, newExchanges, initData, now);
